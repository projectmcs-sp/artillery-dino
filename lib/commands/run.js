      ...
      flow: [
        {
          loop: [
            {
	        "post": {
                    "url": "/api/0.1.0/session",
                    "json": {
                        "username": "andy@bankerslab.com",
                         "password": "qwQ6lkh&",
                         "type": "simple_form"
                    },
                    "headers": {
...
function createScript(opts, parsedURL) {
  var result = JSON.parse(JSON.stringify(posttemplate));
  ...
  result.scenarios[0].flow[0].loop[0].post.url = parsedURL.path;
  return result;
}
